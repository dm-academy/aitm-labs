...
###############################################################################
  ...
  config.vm.define "manos1" do | manos1 |
    ...
    manos1.vm.provision :chef_zero do |chef|
      ...
      chef.add_recipe               "manos1::default"
      if ENV['LAB_NUM']=="lab03"
        puts "Skipping remote git"
      else
        chef.add_recipe               "manos1::git-remote.rb"
      end
    end
  end
....
...
....
==> manos1: Fixed port collision for 22 => 2114. Now on port 2201.
==> manos1: Fixed port collision for 80 => 8114. Now on port 2202.
==> manos1: Fixed port collision for 8080 => 9114. Now on port 2203.
==> manos1: Fixed port collision for 22 => 2222. Now on port 2214.
==> manos1: Clearing any previously set network interfaces...
==> manos1: Preparing network interfaces based on configuration...
    manos1: Adapter 1: nat
==> manos1: Forwarding ports...
    manos1: 22 => 2201 (adapter 1)
    manos1: 80 => 2202 (adapter 1)
    manos1: 8080 => 2203 (adapter 1)
    manos1: 22 => 2214 (adapter 1)
==> manos1: Booting VM...
....
...
"Calavera_manos1_XXXXXXXXXXXXX_XXXXX" {389dab0f-2f52-434e-bf50-c9792c42416a}

Go:

 vboxmanage showvminfo Calavera_manos1_XXXXXXXXXXXXX_XXXXX|more (replacing the X's with the actual numbers, you should cut and paste)
...
Once you have your port 8080 mapping and the Vagrant launching process is complete, you can access your VM's web server. Manos1 comes preconfigured with a running Tomcat instance and a simple test-harness based Java application. You can see it running when you ssh into the VM:
...
....
vagrant@manos1:~$ curl localhost:8080/MainServlet
...
....
YourStudentID@serverXXX:~/Calavera$ tree cookbooks/manos1/
cookbooks/manos1/
├── files
│   ├── build.xml
│   ├── Class1.java
...
Now, the cookbook here includes the raw ingredients (the contents of the cookbooks/manos1/files directory) as well as the recipes of how to set them up on the VM. Especially, have a look at cookbooks/manos1/recipes/default.rb:

....
YourStudentID@serverXXX:~/Calavera$ more  cookbooks/manos1/recipes/default.rb
# manos1-default
...
There is more, but you get the idea. Without going into the https://en.wikipedia.org/wiki/Ruby_(programming_language)[Ruby] code this is written in (which would be too much detail for this class), this script  is creating a set of directory structures on the new manos1 VM and populating them with the basic https://en.wikipedia.org/wiki/Java_(programming_language)[Java] and https://en.wikipedia.org/wiki/Apache_Ant[Ant] files needed. For example, this command:
...
Go back into your manos1 VM and have a look at the home/hijo directory:

....
YourStudentID@serverXXX:~/Calavera$ vagrant ssh manos11
Welcome to Ubuntu 14.04.2 LTS (GNU/Linux 3.13.0-24-generic x86_64)
...
....
vagrant@manos1:/home/hijo$ tree /var/lib/tomcat6/webapps/ROOT/WEB-INF/
...
....
vagrant@manos1:/home/hijo$ nano src/main/java/biz/calavera/MainServlet.java
...
Now try:
    ...
    vagrant@manos1:/home/hijo$ curl localhost:8080/MainServlet
...
Let's add it to git:
....
vagrant@manos1:/home/hijo$ git add src/main/java/biz/calavera/MainServlet.java
vagrant@manos1:/home/hijo$ git commit -m "my local java"
...
....
vagrant@manos1:/home/hijo$ git log -p -1
...
....
vagrant@manos1:/home/hijo$ more src/test/java/biz/calavera/TestClass1.java
package biz.calavera;
...
....
vagrant@manos1:/home/hijo$ nano src/main/java/biz/calavera/Class1.java
...
....
vagrant@manos1:/home/hijo$ cat target/result.txt
...
Let's abandon the changes that "broke the build":
    ...
    vagrant@manos1:/home/hijo$ git reset --hard
...
....
vagrant@manos1:/home/hijo$ git log --pretty=short --abbrev-commit
commit 3b810e4
Author: Charles Betz <char@calavera.biz>
...
....
vagrant@manos1:/home/hijo$ git revert <hash>
....
...
....
vagrant@manos1:/home/hijo$ cat src/main/java/biz/calavera/MainServlet.java
....
