...
* Check the correct box in Putty
...
NOTE: You can exit at this point from your VM to the main server, or just open a new session with X enabled, if you want to see this in firefox. Remember to use your specific mapped 8080 port number that Vagrant setup.
...
Find the git commit hash associated with your commit, and run the revert command:

....
vagrant@manos:/home/hijo$ git revert 3b810e4
....

NOTE: You will need to edit the revert message in nano. You don't need to change the message, so simply exit nano and save the file.
