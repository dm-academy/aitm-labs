...
First, you need to do some setup work, by copying the virtual machine file to your home directory. This prevents a lengthy download process.

....
YourStudentID@serverXXX:~$ mkdir -p .vagrant.d/boxes
YourStudentID@serverXXX:~$ cp -R /var/vagrant/.vagrant.d/boxes/precise64 .vagrant.d/boxes
....
...
Instead of "ls -al," try "tree -al":

....
YourStudentID@serverXXX:~$ tree -al
.
...
If your directory layout does not look the same as above (particularly the vagrant and .vagrant.d directories) check your work, or ask for assistance. (Don't worry about extra files you may have, but be sure the minimum above is present.)

Try:
....
YourStudentID@serverXXX:~$ ls -al .vagrant.d/boxes/precise64/0/virtualbox/
total 313172
drwxr-xr-x 2 YourStudentID student      4096 Sep 19 11:37 .
drwxr-xr-x 3 YourStudentID student      4096 Sep 19 11:37 ..
-rwxr-xr-x 1 YourStudentID student 320654336 Sep 19 11:37 box-disk1.vmdk
-rwxr-xr-x 1 YourStudentID student     14108 Sep 19 11:37 box.ovf
-rwxr-xr-x 1 YourStudentID student        25 Sep 19 11:37 metadata.json
-rwxr-xr-x 1 YourStudentID student       505 Sep 19 11:37 Vagrantfile
....

Notice the size (320654336) of "box-disk1.vmdk." This is the virtual machine image.  (I'd like to find a smaller one, but it's OK for now.)
...
IMPORTANT: If you destroy the VM you will LOSE ALL DATA you have on the machine, unless that data is stored in your ~/vagrant directory (which is mapped to your /vagrant directory in the VM).
...
....
test3@serverXXX:~/example$ git config --global user.email "you@stthomas.edu"
test3@serverXXX:~/example$ git config --global user.name "your name"
....
...
....
test3@serverXXX:~/example$ git commit -m "my first commit"
...
....
test4@seis660:~/example$ vagrant ssh
Welcome to Ubuntu 12.04 LTS (GNU/Linux 3.2.0-23-generic x86_64)
